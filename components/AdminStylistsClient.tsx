...
type AdminStylistsClientProps = {
  initialStylists: Stylist[];
}

export default function AdminStylistsPage( { initialStylists }: AdminStylistsClientProps ) {
  ...
  const fetchStylists = async (nextShowInactive: boolean = showInactive) => {
    try {
      ...
      if (data && !data.error) {
        const filtered = nextShowInactive
          ? data
