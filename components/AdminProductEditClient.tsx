...
type AdminProductEditClientProps = {
  initialProduct: Product;
}

export default function EditProductPage({ initialProduct }: AdminProductEditClientProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    badges: Array.isArray(initialProduct.badges) ? initialProduct.badges.join(", ") : "",
    features: Array.isArray(initialProduct.features) ? initialProduct.features.join("\n") : "",
    specs:
        initialProduct.specs && typeof initialProduct.specs === "object"
        ? JSON.stringify(initialProduct.specs, null, 2)
        : "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const badges = formData.badges
        ? typeof formData.badges === "string" ? formData.badges.split(",").map((b) => b.trim()).filter(Boolean) : []
        : [];
      const features = formData.features
        ? typeof formData.features === "string" ? formData.features.split("\n").filter(Boolean) : []
        : [];
      const images = formData.images
        ? typeof formData.images === "string" ? formData.images.split(",").map((i) => i.trim()).filter(Boolean) : []
        : [];
      
      // specsフィールドのパース（エラーハンドリング付き）
      let specs = {};
      if (formData.specs && typeof formData.specs === "string" && formData.specs.trim()) {
        try {
          specs = JSON.parse(formData.specs as string);
        } catch (parseError) {
              ...
              <div className="mt-4 space-y-4">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    name="badges"
                    value={typeof formData.badges === "string" ? formData.badges : ""}
                    onChange={handleInputChange}
                    className="mt-1 w-full rounded-lg border border-slate-300 px-4 py-2 text-sm focus:border-slate-900 focus:outline-none"
