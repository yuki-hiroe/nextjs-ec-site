...
export default function AdminUsersClient({ initialUsers }: AdminUsersClientProps) {
    ...
    const fetchUsers = async (nextShowSuspended: boolean = showSuspended, nextSearchTerm: string = searchTerm) => {
      try {
        const response = await fetch(
          `/api/admin/users?includeSuspended=${nextShowSuspended}&search=${encodeURIComponent(nextSearchTerm)}`,
          {
            method: "POST",
            headers: {
    ...
    const handleSearch = () => {
      fetchUsers(showSuspended, searchTerm);
    };
            ...
            <label className="flex items-center gap-2">
              <input
                ...
                onChange={(e) => {
                    ...
                    setShowSuspended(checked);
                    fetchUsers(checked, searchTerm);
                }}
